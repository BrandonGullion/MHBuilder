...
for (let i = 0; i < trArray.length; i++) {
    ...
    const getArcShot = (cell) => {
        arcShotArray.push(cell.innerText);
    }
    
    const getShotType = (cell1, cell2, cell3, cell4) => {
        shotTypeArray.push(`${cell1.innerText}/${cell2.innerText}/${cell3.innerText}/${cell4.innerText}`);
    } 

    const  getCoatingType = (cell) => {
        let stringArray = cell.innerText.split("\n");
        let phialString = "";
        for(let i = 0; i < stringArray.length; i++){
            if(i === stringArray.length - 1){
                phialString += `${stringArray[i]}`;
                break;
            }
            phialString += `${stringArray[i]}/`;
        }

        phialOrShellingType.push(phialString);
    } 
    ...
    if (trArray[i].cells.length === 14) {
        ...
        getArcShot(trArray[i].cells.item(4));
        getShotType(trArray[i].cells.item(5),trArray[i].cells.item(6),trArray[i].cells.item(7),trArray[i].cells.item(8));
        getDefense(trArray[i].cells.item(9));
        getRarity(trArray[i].cells.item(10));
        getRampageSkills(trArray[i].cells.item(11));
        getCoatingType(trArray[i].cells.item(12));
    }
}
// Removes the first deco element because its a header
decoArray.shift();
shotTypeArray.shift();
phialOrShellingType.shift();
arcShotArray.shift();
...
function generateObjects() {
    ...
    for (let i = 0; i < nameArray.length; i++) {
        ...
        let weaponObject = {
            ...
            phialOrShellingType: phialOrShellingType[i],
