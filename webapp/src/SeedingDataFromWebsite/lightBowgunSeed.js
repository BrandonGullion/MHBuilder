...
let weaponType = "LightBowgun";
let phialOrShellingType = "None";
let objectArray = [];
let nameArray = [];
let attackArray = [];
let elementArray = [];
...
for (let i = 0; i < trArray.length; i++) {
    ...
    const getBowgunSpecs = (
        ...
        let specsString = `${devCell.innerText}//${recoilCell.innerText}//${reloadCell.innerText}//${clusterCell.innerText}//${specialAmmoCell.innerText}`
        specsArray.push(specsString);
    };

    // If the data cells is 8 (size of table) then move onto next phase
    if (trArray[i].cells.length === 12) {
        ...
        getAffinity(trArray[i].cells.item(2));
        getBowgunSpecs(
            trArray[i].cells.item(3),
            trArray[i].cells.item(4),
            trArray[i].cells.item(5),
            trArray[i].cells.item(6),
            trArray[i].cells.item(7)
        );

        getDefense(trArray[i].cells.item(8)); 
        getRarity(trArray[i].cells.item(9));
        getRampageSkills(trArray[i].cells.item(10));
    }
}
...
function generateObjects() {
    ...
    for (let i = 0; i < nameArray.length; i++) {
        ...
        let weaponObject = {
            ...
            phialOrShellingType: phialOrShellingType,
            bowGunSpecs: specsArray[i],
