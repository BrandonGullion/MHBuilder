...
for (let i = 0; i < trArray.length; i++) {
    ...
    const getShellingType = (cell) => {
        shellingArray.push(cell.innerText);
    }

    const getShellingLvl = (cell) => {
        shellingLvl.push(cell.innerText.split(" ")[1]);
    }
    ...
    if (trArray[i].cells.length === 10) {
        ...
        getDefense(trArray[i].cells.item(4));
        getShellingType(trArray[i].cells.item(5));
        getShellingLvl(trArray[i].cells.item(6))
        getRarity(trArray[i].cells.item(7));
        getRampageSkills(trArray[i].cells.item(8));
    }
}
...
function generateObjects() {
    ...
    for (let i = 0; i < nameArray.length; i++) {
        ...
        let weaponObject = {
            ...
            phialOrShellingType: shellingArray[i],
            shellingLevel: shellingLvl[i],
